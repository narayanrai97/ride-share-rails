...
RSpec.describe "Api::V1::Schedule_Windows", type: :request do
    ...
    it "Gets schedule_window by ID" do
        get "/api/v1/availabilities/window/#{recurring_pattern.schedule_window.id}", headers: headers, params: {
            ...
            # check that start times are correct
             startTime = parsed_json['json'].map{|k| k["startTime"] }
             expect(startTime).to eq(["2019-09-21 14:00", "2019-09-14 14:00", "2019-09-07 14:00"])
             
          
            #check that end times are correct
            endTime = parsed_json['json'].map{|k| k['endTime'] }
            expect(endTime).to eq(["2019-09-21 16:00", "2019-09-14 16:00", "2019-09-07 16:00"])
        ...
        params: { 
          ...
          end_date: "2019-10-21", 
          start_time: "2019-09-01 1500", 
          end_time: "2019-09-01 1700", 
          is_recurring: true,
          location_id: location.id
      } 
        parsed_json = JSON.parse(response.body)
        expect(parsed_json["json"]["start_date"]).to eq("2019-09-01T00:00:00.000Z")
        expect(parsed_json["json"]["end_date"]).to eq("2019-10-21T00:00:00.000Z")
        expect(parsed_json["json"]["start_time"]).to eq("2019-09-01T00:00:00.000Z")
        expect(parsed_json["json"]["end_time"]).to eq("2019-09-01T00:00:00.000Z")
